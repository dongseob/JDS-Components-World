...
/**
 * transition modal - fade in out
 */
const ModalFade = ({ isModalFade, setIsModalFade }) => {
    ...
    const modalCloseOutside = (e) => {
        if (modalRef.current === e.target || buttonRef.current === e.target) {
            ...
            modalChildRef.current.style.transform = "translateY(-20px)";

            //modal animation delay function
            setTimeout(() => {
                setIsModalFade(false);
            }, 500);
        } else {
            setIsModalFade(true);
        }
    ...
    return (
        <>
            {/* modal background */}
            <div
                className="fixed left-0 top-0 w-full h-full bg-gray-500/75 flex justify-center items-center z-30 animate-fade-in"
                ref={modalRef}
                onClick={(e) => modalCloseOutside(e)}
            >
                {/* modal wrap */}
                <div
                    className="bg-white w-1/4 min-w-[400px] h-auto rounded-lg p-6 flex items-center flex-col shadow-lg animate-fade-in-down"
                    ref={modalChildRef}
                >
                    {/* modal header */}
                    <h3>
