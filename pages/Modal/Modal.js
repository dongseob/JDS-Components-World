...
const Modal = () => {
    ...
    return (
        <div className="wrap">
            ...
            <div className="text-2xl font-semibold mt-2">Modal</div>
            <div className="card">
                <div className="mt-0 font-semibold">Basic Modal</div>
                <div className="text-sm mt-4">
                    * 기존의 TailwindCSS의 Modal의 경우 Headless UI를 사용해야
                    하므로 Plane하게 구현
                </div>
                <button
                    className="button mt-4"
                    onClick={(e) => setIsModalBasic(true)}
                >
                    open modal
                </button>
            </div>
            ...
            {isModalBasic && (
                <ModalBasic
                    isModalBasic={isModalBasic}
                    setIsModalBasic={setIsModalBasic}
                />
            )}
            {isModalFade && (
                <ModalFade
                    isModalFade={isModalFade}
                    setIsModalFade={setIsModalFade}
                />
            )}
            {isModalZoom && (
                <ModalZoom
                    isModalZoom={isModalZoom}
                    setIsModalZoom={setIsModalZoom}
                />
            )}
        </div>
    );
