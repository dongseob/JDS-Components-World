...
const SearchAuto = () => {
    ...
    // 검색 함수
    const fnSubmit = (e, type, val) => {
        e.preventDefault();
        if (type === "enter") {
            alert(inputRef.current.value + "로 검색 하셨네요.");
        } else if (type === "list") {
            alert(val + "로 검색 하셨네요.");
        }
    };
    ...
    const fnChange = (e) => {
        if (e.target.value.length > 0) {
            // 한글자 이상이라도 있다면 자동검색 리스트에서 조회
            setAutoReady(true);
            const filteredList = list.filter((el) =>
                el.includes(e.target.value)
            );
            const filteredResult = filteredList.map((el, idx) => {
                console.log(el);
                return el;
            });
    ...
    return (
        <div className="card">
            <div className="mt-0 font-semibold">검색어 자동완성</div>
            <div className="mt-4">
                <form
                    onSubmit={(e) => fnSubmit(e, "enter")}
            ...
            <div className="px-[18px] my-[24px]">
                <p className="font-bold">자동완성 목록</p>
                {autoReady && result !== null && (
                    <ul className="mt-[12px]">
                        {result.map((el, idx) => (
                            <li
                                key={idx}
                                className="py-[12px] border-b flex relative"
                                onClick={(e) => fnSubmit(e, "list", el)}
                            >
                                {el}
                            </li>
                        ))}
